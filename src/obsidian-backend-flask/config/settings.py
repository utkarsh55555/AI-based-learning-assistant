...
# Set FLASK_DEBUG=True in .env to see debug-level output in development.
_log_level = (
    logging.DEBUG
    if os.getenv("FLASK_DEBUG", "False").lower() in ("1", "true")
...
class Settings:
    ...
    # Audit log — file path relative to backend dir
    ...
    def validate(cls, strict: bool = False):
        ...
        if not os.getenv("SECRET_KEY"):
            security_warnings.append(
                "SECRET_KEY is not set; using an ephemeral random value "
        ...
        if missing:
            ...
            if strict:
                ...
                logger.warning(
                    "Some features may not work. Please add them to "
